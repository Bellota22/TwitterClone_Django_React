...
import { userProfile, follow } from "../api/user"
...
function UserProfile() {
    ...
    if(loadingTweets || loadingUser) return <Loader />
    if (isErrorUser  || isErrorTweets) return toast.error('Error')

  return (
   <>
    
    {isEditing && (
      <EditProfile user={user} close={() => setIsEditing(!isEditing)} />
        )}
      ...
      <img className="bg-black h-[250px] w-full object-cover " src={user.cover_image} />
      ...
      <div className="border-b-[1px] border-neutral-800 grid grid-cols-4 gap-4">
        ...
        {show === 0 && <MyTweets user={user} myUser={myUser} tweets={tweets} />}
